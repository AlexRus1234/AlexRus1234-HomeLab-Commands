...
### 1.2 Форматирование и монтирование дисков (Stateless)
У ВМ должны быть добавлены два дополнительных виртуальных диска по 1 ГБ (под БД и под Конфиги).
```bash
...
# Узнаем UUID дисков (СКОПИРУЙТЕ ИХ!)
sudo blkid /dev/sdb /dev/sdc
```

Добавляем диски в автозагрузку. Обратите внимание на флаг `noatime` для диска с базами данных:
```bash
...
UUID=ваш_uuid_от_sdc  /opt/appdata/config  ext4  defaults,discard          0  2
```
Применяем настройки:
```bash
sudo mount -a
df -h  # Проверяем, что диски примонтированы
```
...
# Отдаем ему права на наши новые диски
...
# Перезагружаем ВМ, чтобы ядро корректно подхватило диски и юзеров
sudo reboot
```
...
```bash
# Папка для Quadlets на внешнем диске
mkdir -p /opt/appdata/config/quadlets

# Стандартный системный путь для Rootless Podman
mkdir -p ~/.config/containers

# Создаем ярлык (symlink), чтобы systemd читал файлы с нашего диска
